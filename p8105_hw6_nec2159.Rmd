...
In homicide cases in Baltimore, MD, male victims had `r baltimore_results |> pull(estimate)` times the odds of having their cases solved compared to female victims after adjusting for age and race. We are 95% confident that the true odds ratio for male versus female victims is between `r baltimore_results |> pull(conf.low)` and `r baltimore_results |> pull(conf.high)`.
...
In the 5000 bootstrap samples, $\frac{\hat{\beta}_1}{\hat{\beta}_2}$ appears within (`r estimates |> pull(beta_ratio) |> quantile(c(.025, .975))`) 95% of the time, and $R^2$ appears within (`r estimates |> pull(r_squared) |> quantile(c(.025, .975))`) 95% of the time.
...
LASSO was used to build a regression model for birth weight as it performs variable selection and coefficient shrinkage. The data was first converted into numerical format with x as the predictors and y as the outcome. A number of lambda values along with cross validation was then used to determine the optimal lambda which determines the amount of shrinkage applied to each coefficient. This is the lambda that was then used to fit the proposed regression model. Predictors include `bhead`, `blength`, `delwt`, and `mrace`.

```{r}
lasso_model = lm(bwt ~ bhead + blength + delwt + mrace, data = bwt_df)
...
The residuals versus fitted values plot shows the model is appropriate because the residuals do not exhibit any pattern and are randomly scattered around 0. 
...
The interactions model has the lowest prediction error, indicating `bhead * blength * babysex` predicts birth weight the most accurately compared to the LASSO model and main effects model.
